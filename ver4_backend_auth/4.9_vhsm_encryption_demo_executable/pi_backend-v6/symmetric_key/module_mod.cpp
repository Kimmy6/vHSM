...
extern "C" int extract_bits_from_image(const char *image_path, uint8_t *out_bits, int max_len) {
    ...
    // Bug fix 1: split into two separate system() calls (single \n broke dcraw execution)
    // Bug fix 2: -o capture.jpg so that --raw produces capture.dng (not capture.tiff.dng)
    printf("Image capturing...\n");

    int ret = system("rpicam-still --raw --timeout 50 --shutter 1500 --gain 1.0 -o capture.jpg");
    if (ret != 0) {
        fprintf(stderr, "Image Capture Failure!\n");
        return 1;
    }

    ret = system("dcraw -4 -d -T capture.dng");
    if (ret != 0) {
        fprintf(stderr, "dcraw RAW conversion Failure!\n");
        return 1;
    }
