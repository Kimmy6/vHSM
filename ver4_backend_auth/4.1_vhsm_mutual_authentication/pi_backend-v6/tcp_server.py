...
def ensure_pi_keypair(user_id: str) -> tuple[Path, Path]:
    ...
    # [BUG 2 FIX] keygen 바이너리가 요구하는 cert_pem_out 경로 추가
    ...
    # mldsa_keygen <seed_file> <priv_out> <pub_out> <cert_pem_out> <subject_cn> [country] [org]
    result = run_checked(
        [
            str(KEYGEN_BINARY),
            str(seed_file),
            ...
            str(public_key_path),
            str(cert_key_path),  # [BUG 2 FIX] cert_pem_out — 기존에 누락되어 keygen이 항상 실패
            safe_user,           # [BUG 2 FIX] subject_cn
        ],
        MLDSA_DIR,
    )
...
def handle_key_command(user_id: str, mode: str, key_number: str) -> str:
    ...
    if not ok:
        log_backend_error(f"Arduino 키 명령 실패 - user={user_id}, mode={normalized_mode}, key={normalized_key}, detail={detail}")
...
def handle_command(command: str) -> str:
    ...
    # [BUG 1 FIX] KEY_COMMAND 블록이 함수 바깥으로 탈출해 있던 들여쓰기 오류 수정
        ...
        while True:
            ...
            with conn:
                try:
                    ...
                    log_backend_info(
                        f"명령 수신 - client={client_ip}:{client_port}, command={data[:200]}"
                    )

                    reply = handle_command(data)

                    log_backend_info(
                        f"응답 전송 - client={client_ip}:{client_port}, reply={reply[:120]}"
                    )
                    ...
                    log_backend_error(
                        f"클라이언트 처리 중 예외 - client={client_ip}:{client_port}, detail={exc}"
                    )
                    try:
