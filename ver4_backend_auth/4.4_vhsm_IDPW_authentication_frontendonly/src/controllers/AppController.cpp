...
void AppController::goToSignUpPage()       { emit openSignUpPage(); }
void AppController::goToFindIdPage()       { emit openFindIdPage(); }
void AppController::goToFindPasswordPage() { emit openFindPasswordPage(); }

// ─────────────────────────────────────────────
// 회원 가입 (스텁 — 백엔드 연동 예정)
// ─────────────────────────────────────────────

void AppController::signUp(const QString &id,
                           const QString &password,
                           const QString &passwordConfirm,
                           const QString &name,
                           const QString &email)
{
    if (id.trimmed().isEmpty() || password.isEmpty() || name.trimmed().isEmpty()) {
        m_state.setSignUpStatusMessage(QStringLiteral("모든 필수 항목을 입력해주세요."));
        return;
    }
    if (password != passwordConfirm) {
        m_state.setSignUpStatusMessage(QStringLiteral("비밀번호가 일치하지 않습니다."));
        return;
    }
    // TODO: 백엔드 연동
    m_state.setSignUpStatusMessage(QStringLiteral("회원 가입이 완료되었습니다."));
}
...
void AppController::findId(const QString &name)
{
    if (name.trimmed().isEmpty()) {
        m_state.setFindIdStatusMessage(QStringLiteral("이름을 입력해주세요."));
        return;
    }
    // TODO: 백엔드 연동
    m_state.setFindIdStatusMessage(QStringLiteral("일치하는 계정을 찾을 수 없습니다."));
}
...
void AppController::findPassword(const QString &name, const QString &id)
{
    if (name.trimmed().isEmpty() || id.trimmed().isEmpty()) {
        m_state.setFindPasswordStatusMessage(QStringLiteral("이름과 아이디를 모두 입력해주세요."));
        return;
    }
    // TODO: 백엔드 연동 후 ResetPasswordPage로 이동
    m_state.setFindPasswordStatusMessage(QStringLiteral("일치하는 계정을 찾을 수 없습니다."));
}
...
{
    if (password.isEmpty()) {
        m_state.setResetPasswordStatusMessage(QStringLiteral("비밀번호를 입력해주세요."));
        return;
    }
    if (password != passwordConfirm) {
        m_state.setResetPasswordStatusMessage(QStringLiteral("비밀번호가 일치하지 않습니다."));
        return;
    }
    // TODO: 백엔드 연동
    m_state.setResetPasswordStatusMessage(QStringLiteral("비밀번호 재설정이 완료되었습니다."));
}
