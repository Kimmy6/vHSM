...
def ensure_pi_keypair(user_id: str) -> tuple[Path, Path]:
    ...
    cert_key_path = user_dir / "pi_mldsa_cert.pem"  # [BUG 2 FIX] cert_pem_out 경로 추가
    ...
    # [BUG 2 FIX] cert_pem_out + subject_cn 인자 추가 (기존에 누락되어 keygen이 항상 실패)
    result = run_checked(
        [
            str(KEYGEN_BINARY),
            str(seed_file),
...
def start_auth(user_id: str, nonce_b64: str) -> str:
    """
    단방향 인증 (가상HSM 진위 검증):
      앱 → Pi : START_AUTH:<userId>:<nonce_b64>
      Pi → 앱 : AUTH_OK:<pi_sig_b64>
    앱은 사전 저장된 Pi 공개키(PROVISION 단계)로 서명을 검증한다.
    """
    log_backend_info(f"단방향 인증 요청 수신 - user={user_id}")
    ...
    try:
        pi_private_key_path, _ = ensure_pi_keypair(user_id)
        base64.b64decode(nonce_b64, validate=True)
    except Exception as exc:
        log_backend_error(f"단방향 인증 준비 실패 - user={user_id}, detail={exc}")
        return "ERROR: AUTH_FAILED"
    ...
    log_backend_info(f"단방향 인증 서명 완료 - user={user_id}")
...
def handle_key_command(user_id: str, mode: str, key_number: str) -> str:
    ...
    log_backend_info(f"Arduino 키 명령 완료 - user={user_id}, mode={normalized_mode}, key={normalized_key}, pin={pin_label}")
    return "KEY_OK"
...
def handle_command(command: str) -> str:
    ...
    if command.startswith("START_AUTH:"):
        ...
        if len(parts) != 3:
            ...
            return "ERROR: INVALID_COMMAND"
        user_id = parts[1].strip() or "User"
        nonce_b64 = parts[2].strip()
        return start_auth(user_id, nonce_b64)

    # [BUG 1 FIX] KEY_COMMAND가 함수 바깥으로 탈출해 있던 들여쓰기 오류 수정
    if command.startswith("KEY_COMMAND:"):
        ...
        return handle_key_command(user_id, mode, key_number)
        ...
        while True:
            ...
            with conn:
                try:
                    ...
                    if not data:
                        log_backend_error(f"빈 요청 또는 즉시 종료 - client={client_ip}:{client_port}")
                        continue
