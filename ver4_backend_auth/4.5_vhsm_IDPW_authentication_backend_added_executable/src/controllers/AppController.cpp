...
void AppController::beginTlsSessionSequence()
{
    ...
    // 성공 — ConnectPage로 돌아가며 상태 메시지 표시
...
void AppController::goBackToConnectPage()                { m_state.setRegenerateStatusMessage(QString()); m_state.setLatestPublicKey(QString()); emit backToConnectPage(); }
...
void AppController::startEncryptWithKey(const QString &keyNumber)
{
    ...
    QString err;
    if (!m_piGatewayService.sendKeyCommand(m_state.currentUser(), m_state.currentPiHost(), QStringLiteral("encrypt"), t, &err)) {
        m_state.setStatusMessage(err.isEmpty() ? QStringLiteral("암호화 명령 전송 실패") : err); return;
    }
    const QString pin = t==QStringLiteral("1")?QStringLiteral("D3"):t==QStringLiteral("2")?QStringLiteral("D4"):QStringLiteral("D5");
    m_state.setStatusMessage(QStringLiteral("암호화 명령을 전송했습니다. 키 %1 → Arduino %2 ON").arg(t, pin));
}
...
void AppController::startDecryptWithKey(const QString &keyNumber)
{
    ...
    QString err;
    if (!m_piGatewayService.sendKeyCommand(m_state.currentUser(), m_state.currentPiHost(), QStringLiteral("decrypt"), t, &err)) {
        m_state.setStatusMessage(err.isEmpty() ? QStringLiteral("복호화 명령 전송 실패") : err); return;
    }
    const QString pin = t==QStringLiteral("1")?QStringLiteral("D3"):t==QStringLiteral("2")?QStringLiteral("D4"):QStringLiteral("D5");
    m_state.setStatusMessage(QStringLiteral("복호화 명령을 전송했습니다. 키 %1 → Arduino %2 ON").arg(t, pin));
}
