                                         ...
                                         QString *errorMessage) const
{
    try {
        ...
        if (errorMessage)
            *errorMessage = QStringLiteral("ML-DSA 키 생성 예외: %1").arg(QString::fromUtf8(e.what()));
        return false;
    }
}
                               ...
                               QString *errorMessage) const
{
    try {
        ...
        if (errorMessage)
            *errorMessage = QStringLiteral("스마트폰 서명 예외: %1").arg(QString::fromUtf8(e.what()));
        return false;
    }
}
                                 ...
                                 QString *errorMessage) const
{
    try {
        ...
        Botan::PK_Verifier verifier(*publicKey, "Randomized");
        const std::vector<uint8_t> messageBytes(message.begin(), message.end());
        ...
        if (errorMessage)
            *errorMessage = QStringLiteral("서명 검증 예외: %1").arg(QString::fromUtf8(e.what()));
        return false;
    }
}
