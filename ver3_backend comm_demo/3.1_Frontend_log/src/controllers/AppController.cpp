...
bool AppController::connectToHsm(const QString &userId)
{
    ...
    m_state.setConnectionPhase(QStringLiteral("가상 HSM 접속 준비 중..."));
...
void AppController::beginConnectionSequence()
{
    ...
    updateConnectionPhase(QStringLiteral("Pi 터미널 연결 요청 중..."));

    QTimer::singleShot(900, this, [this]() {
        appendTerminalLine(QStringLiteral("RSA 인증 중..."));
        updateConnectionPhase(QStringLiteral("RSA 인증 진행 중..."));
    });

    QTimer::singleShot(2200, this, [this]() {
        appendTerminalLine(QStringLiteral("RSA 인증 완료!"));
        updateConnectionPhase(QStringLiteral("인증 완료. 세션 진입 준비 중..."));
    });
