...
bool AppController::connectToHsm(const QString &userId, const QString &piHostOrName)
{
    if (!m_authService.connectUser(userId)) {
        m_state.setStatusMessage("유저명을 입력해주세요.");
        return false;
    }

    QString errorMessage;
    if (!m_piGatewayService.requestUserConnectStart(userId, piHostOrName, &errorMessage)) {
        m_state.setStatusMessage(errorMessage.isEmpty() ? QStringLiteral("입력하신 IP 주소를 찾을 수 없습니다.") : errorMessage);
        return false;
    }
    ...
    m_state.setConnectionPhase(QStringLiteral("가상 HSM 접속 중"));
...
{
    updateConnectionPhase(QStringLiteral("가상 HSM 접속 중"));
