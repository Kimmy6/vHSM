                                        ...
                                        QString *errorMessage) const
{
    ...
    if (!sendCommand(piHostOrName,
                     QStringLiteral("AUTH_USER:%1").arg(trimmedUser),
                     &reply,
                     errorMessage)) {
        return false;
    }
    ...
    if (errorMessage)
        *errorMessage = reply.startsWith(QStringLiteral("ERROR:"))
            ? reply.mid(QStringLiteral("ERROR:").size()).trimmed()
            : QStringLiteral("ML-DSA 인증에 실패했습니다: %1").arg(reply);
    return false;
}
                                           ...
                                           QString *errorMessage) const
{
    ...
    if (!reply.startsWith(prefix)) {
        if (errorMessage)
            *errorMessage = reply.startsWith(QStringLiteral("ERROR:"))
                ? reply.mid(QStringLiteral("ERROR:").size()).trimmed()
                : QStringLiteral("공개키 재생성 응답이 올바르지 않습니다.");
        return false;
    }
