...
void AppController::beginConnectionSequence()
{
    ...
    const QByteArray piPayload = buildPiAuthPayload(userId, sessionId, clientChallenge, serverChallenge);
    ...
    const QByteArray phonePayload = buildPhoneAuthPayload(userId, sessionId, clientChallenge, serverChallenge);
