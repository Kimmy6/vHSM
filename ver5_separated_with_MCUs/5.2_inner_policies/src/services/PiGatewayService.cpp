                                  ...
                                  QString *outRole)
{
    ...
    if (reply.startsWith(QStringLiteral("LOGIN_OK"))) {
        // LOGIN_OK:<role> 형식으로 role 추출
        const int colon = reply.indexOf(QLatin1Char(':'));
        if (outRole && colon >= 0)
            *outRole = reply.mid(colon + 1).trimmed();
        else if (outRole)
            *outRole = QStringLiteral("public_user");
        if (errorMessage) errorMessage->clear();
        return true;
    }
...
bool PiGatewayService::bootstrapRootOfficer(const QString &userId,
                                              const QString &password,
                                              const QString &name,
                                              const QString &email,
                                              const QString &piHostOrName,
                                              QString *errorMessage) const
{
    const QString command = QStringLiteral("BOOTSTRAP:%1:%2:%3:%4")
        .arg(userId.trimmed(),
    ...
    if (!sendCommand(piHostOrName, command, &reply, errorMessage))
        return false;
    if (reply == QStringLiteral("BOOTSTRAP_OK")) { if (errorMessage) errorMessage->clear(); return true; }
    if (errorMessage)
        *errorMessage = reply.contains(QStringLiteral("BOOTSTRAP_NOT_ALLOWED"))
            ? QStringLiteral("HSM이 이미 초기화되어 있습니다.")
            : QStringLiteral("초기화 실패: ") + reply;
    return false;
}

bool PiGatewayService::createSlot(const QString &rootId,
                                   const QString &slotName,
                                   const QString &piHostOrName,
                                   QString *errorMessage)
{
    const QString command = QStringLiteral("CREATE_SLOT:%1:%2")
        .arg(rootId.trimmed(),
             QString::fromLatin1(slotName.trimmed().toUtf8().toBase64()));
    QString reply;
    if (!sendSessionCommand(command, &reply, errorMessage))
        return false;
    if (reply.startsWith(QStringLiteral("CREATE_SLOT_OK:"))) {
        if (errorMessage) errorMessage->clear();
        return true;
    }
    if (errorMessage) *errorMessage = QStringLiteral("슬롯 생성 실패: ") + reply;
    return false;
}

bool PiGatewayService::createUserInSlot(const QString &rootId,
                                         ...
                                         QString *errorMessage)
{
    ...
    const QString command = QStringLiteral("CREATE_USER:%1:%2:%3:%4:%5:%6:%7")
        .arg(rootId.trimmed(), slotId.trimmed(), newUserId.trimmed(),
             QString::fromLatin1(password.toUtf8().toBase64()),
             QString::fromLatin1(name.trimmed().toUtf8().toBase64()),
             QString::fromLatin1(email.trimmed().toUtf8().toBase64()),
             QString::fromLatin1(role.trimmed().toUtf8().toBase64()));
    QString reply;
    if (!sendSessionCommand(command, &reply, errorMessage))
        return false;
    if (reply == QStringLiteral("CREATE_USER_OK")) { if (errorMessage) errorMessage->clear(); return true; }
    if (errorMessage)
        *errorMessage = reply.contains(QStringLiteral("ALREADY_EXISTS"))
            ? QStringLiteral("이미 존재하는 아이디입니다.")
            : reply.contains(QStringLiteral("SLOT_NOT_FOUND"))
                ? QStringLiteral("슬롯을 찾을 수 없습니다.")
                : QStringLiteral("유저 생성 실패: ") + reply;
    return false;
}

bool PiGatewayService::generateInvite(const QString &rootId,
                                       const QString &slotId,
                                       const QString &role,
                                       int expireDays,
                                       const QString &piHostOrName,
                                       ...
                                       QString *errorMessage)
{
    ...
    const QString command = QStringLiteral("GENERATE_INVITE:%1:%2:%3:%4")
        .arg(rootId.trimmed(),
             QString::fromLatin1(slotId.trimmed().toUtf8().toBase64()),
             QString::fromLatin1(role.trimmed().toUtf8().toBase64()),
             QString::number(expireDays));
    QString reply;
    if (!sendSessionCommand(command, &reply, errorMessage))
        return false;
    const QString prefix = QStringLiteral("INVITE_OK:");
    if (reply.startsWith(prefix)) {
        if (outCode) *outCode = reply.mid(prefix.size()).trimmed();
        if (errorMessage) errorMessage->clear();
        return true;
    }
    if (errorMessage)
        *errorMessage = reply.contains(QStringLiteral("SLOT_NOT_FOUND"))
            ? QStringLiteral("슬롯을 찾을 수 없습니다.")
            : reply.contains(QStringLiteral("INVALID_ROLE"))
                ? QStringLiteral("유효하지 않은 역할입니다.")
                : QStringLiteral("초대 코드 생성 실패: ") + reply;
    return false;
}

bool PiGatewayService::registerWithInvite(const QString &inviteCode,
                                           const QString &newUserId,
                                           const QString &password,
                                           const QString &name,
                                           const QString &email,
                                           ...
                                           QString *errorMessage) const
{
    ...
    const QString command = QStringLiteral("REGISTER_WITH_INVITE:%1:%2:%3:%4:%5")
        .arg(inviteCode.trimmed().toUpper(),
             newUserId.trimmed(),
             QString::fromLatin1(password.toUtf8().toBase64()),
             QString::fromLatin1(name.trimmed().toUtf8().toBase64()),
             QString::fromLatin1(email.trimmed().toUtf8().toBase64()));
    ...
    if (!sendCommand(piHostOrName, command, &reply, errorMessage))
        return false;
    if (reply == QStringLiteral("REGISTER_WITH_INVITE_OK")) {
        if (errorMessage) errorMessage->clear();
        return true;
    }
    if (errorMessage) {
        if (reply.contains(QStringLiteral("INVALID_CODE")))
            *errorMessage = QStringLiteral("유효하지 않은 초대 코드입니다.");
        else if (reply.contains(QStringLiteral("CODE_ALREADY_USED")))
            *errorMessage = QStringLiteral("이미 사용된 초대 코드입니다.");
        else if (reply.contains(QStringLiteral("CODE_EXPIRED")))
            *errorMessage = QStringLiteral("만료된 초대 코드입니다.");
        else if (reply.contains(QStringLiteral("ALREADY_EXISTS")))
            *errorMessage = QStringLiteral("이미 사용 중인 아이디입니다.");
        else
            *errorMessage = QStringLiteral("가입 실패: ") + reply;
    }
    return false;
}

bool PiGatewayService::listSlots(const QString &rootId,
                                  const QString &piHostOrName,
                                  QString *outSlotJson,
                                  QString *errorMessage)
{
    const QString command = QStringLiteral("LIST_SLOTS:%1").arg(rootId.trimmed());
    QString reply;
    if (!sendSessionCommand(command, &reply, errorMessage))
        return false;
    const QString prefix = QStringLiteral("LIST_SLOTS_OK:");
    if (reply.startsWith(prefix)) {
        // Base64 디코딩 → JSON 문자열 반환
        const QByteArray decoded = QByteArray::fromBase64(
            reply.mid(prefix.size()).toUtf8());
        if (outSlotJson) *outSlotJson = QString::fromUtf8(decoded);
        if (errorMessage) errorMessage->clear();
        return true;
    }
    if (errorMessage) *errorMessage = QStringLiteral("슬롯 목록 조회 실패: ") + reply;
    return false;
}
