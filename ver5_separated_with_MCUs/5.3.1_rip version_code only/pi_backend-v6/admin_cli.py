...
def add_user_to_existing_slot(data: dict) -> None:
    divider("기존 슬롯에 유저 추가")
    ...
    for i, sid in enumerate(slot_ids, 1):
        slot      = data["slots"][sid]
        allowed   = slot.get("allowed_roles", [])
        user_cnt  = len(slot.get("users", {}))
        allowed_str = ", ".join(allowed) if allowed else "전체"
        print(f"  [{i}] {slot['name']}  ({sid})")
        print(f"       유저 수   : {user_cnt}명")
        print(f"       허용 역할 : {allowed_str}")
        print()

    slot_choice = input("  슬롯 번호 선택: ").strip()
    try:
...
def run(user_id: str) -> None:
    ...
    while True:
        ...
        if choice == "1":
            ...
            add_user_to_existing_slot(data)
        elif choice == "0":
            print("\n종료합니다.")
            break
        else:
