...
class AppController : public QObject
{
    ...
    Q_PROPERTY(QString cryptoResult              READ cryptoResult              NOTIFY cryptoResultChanged)

    // 결과 파일 경로
    Q_PROPERTY(QString resultFilePath            READ resultFilePath            NOTIFY resultFilePathChanged)
...
public:
    ...
    // 이미지 암호화: keyNumber(1-3), selectedFile에서 이미지 읽어 암호화 후 Pictures에 저장
    Q_INVOKABLE void startEncryptWithKey(const QString &keyNumber);
    // 이미지 복호화: keyNumber(1-3), selectedFile(.enc)에서 읽어 복호화 후 Pictures에 저장
...
private:
    ...
    // 중복 실행 방지 — 워커 스레드 시작/종료 시 main thread에서만 토글
