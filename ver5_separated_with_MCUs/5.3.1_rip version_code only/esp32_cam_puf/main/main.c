...
void app_main(void)
{
    // IDF v5.x: getchar() 기본 blocking → O_NONBLOCK으로 전환
    // 없으면 첫 READY 후 getchar()에서 무한 대기, 두 번째 READY 안 찍힘
    ...
    while (1) {
        ...
        while (1) {
            ...
            // 줄 단위 읽기: 첫 바이트 도착 후 50ms idle 타임아웃
            // getchar()가 즉시 EOF를 반환해도 바이트가 올 때까지 재시도
            char line[16] = {0};
            int  idx = 0;
            ...
            while (idx < (int)(sizeof(line) - 1)) {
                int c = getchar();
                if (c < 0) {
                    // 버퍼 비어있음 — 50ms 이내면 계속 대기
                    if ((xTaskGetTickCount() - idle_start) >= pdMS_TO_TICKS(50)) break;
                    vTaskDelay(pdMS_TO_TICKS(1));
                    continue;
                }
                idle_start = xTaskGetTickCount();  // 문자 수신 → 타임아웃 리셋
                if (c == '\n') break;
